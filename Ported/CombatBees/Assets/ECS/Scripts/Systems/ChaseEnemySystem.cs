...
public class ChaseEnemySystem : SystemBase
{
    [BurstCompile]
    struct ChaseEnemySystemJob : IJobChunk
    {
        ...
        public ComponentTypeHandle<CTarget> TargetType;
        ...
        public void Execute(ArchetypeChunk chunk, int chunkIndex, int firstEntityIndex)
        {
            ...
            {
                Debug.Assert(AllTranslations.HasComponent(enemies[i]));

                if (AllSecondsToDeath.HasComponent(enemies[i]))
                {
                    CommandBuffer.RemoveComponent<CTarget>(chunkIndex, entities[i]);
                    CommandBuffer.RemoveComponent<TEnemyTarget>(chunkIndex, entities[i]);
                    continue;
                }
    ...
    protected override void OnCreate()
    {
        ...
        {
            All = new ComponentType[] { ComponentType.ReadOnly<Translation>(), ComponentType.ReadWrite<CVelocity>(), ComponentType.ReadWrite<CTarget>(), ComponentType.ReadOnly<TEnemyTarget>() },
    ...
    protected override void OnUpdate()
    {
        Dependency = new ChaseEnemySystemJob
        {
            ...
            CommandBuffer = mCommandBufferSystem.CreateCommandBuffer().AsParallelWriter(),
            EntityType = GetEntityTypeHandle(),
            TranslationType = GetComponentTypeHandle<Translation>(true),
            VelocityType = GetComponentTypeHandle<CVelocity>(false),
            TargetType = GetComponentTypeHandle<CTarget>(false),
