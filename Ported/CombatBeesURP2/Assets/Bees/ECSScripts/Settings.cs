...
public class Settings : MonoBehaviour
{
    ...
    public void Awake()
    {
        ...
        buffer.AddComponent(firstTeamRequest, new CRandomizer { Value = new Unity.Mathematics.Random(math.asuint(random.Next(1, int.MaxValue))) });
        buffer.AddComponent(firstTeamRequest, new CSpawnRequest { Count = BeeCount / 2, Team = 0 });

        var secondTeamRequest = buffer.CreateEntity();
        buffer.AddComponent(secondTeamRequest, new CRandomizer { Value = new Unity.Mathematics.Random(math.asuint(random.Next(1, int.MaxValue))) });
        buffer.AddComponent(secondTeamRequest, new CSpawnRequest { Count = BeeCount / 2, Team = 1 });
